...
function Key-Event([string]$key,[bool]$up=$false){$vk=[byte][int][Windows.Forms.Keys]::$key;$flags=if($up){2}else{0};[MNative]::keybd_event($vk,0,$flags,[UIntPtr]::Zero)}
...
$form=New-Object Windows.Forms.Form;$form.Text='CyberPW Macro Studio · Visual 0.3';$form.ClientSize='1180,750';$form.MinimumSize='1000,670';$form.StartPosition='CenterScreen';$form.BackColor=$base;$form.ForeColor=$text;$form.Font=New-Object Drawing.Font('Segoe UI',9)
...
$check.Add_Click({try{$s=Compile;Set-Status "СЦЕНАРІЙ КОРЕКТНИЙ`r`nКоманд: $($s.Count)" $good}catch{Set-Status $_.Exception.Message $bad}});$new.Add_Click({if(-not$script:running){$name.Text='Новий макрос';$grid.Rows.Clear();Set-Status 'НОВИЙ СЦЕНАРІЙ' $good}})
$save.Add_Click({try{[void](Compile);$safe=$name.Text.Trim()-replace'[\\/:*?"<>|]','_';if(-not$safe){$safe='Новий макрос'};$rows=@();foreach($r in $grid.Rows){$rows+=[ordered]@{command=$r.Cells[0].Value;argument=$r.Cells[1].Value;description=$r.Cells[2].Value}};[ordered]@{schemaVersion=2;name=$safe;targetProcess=$target.Text.Trim();steps=$rows}|ConvertTo-Json -Depth 5|Set-Content -Encoding UTF8 (Join-Path $macroDir ($safe+'.json'));Set-Status 'ЗБЕРЕЖЕНО' $good}catch{Set-Status $_.Exception.Message $bad}})
$open.Add_Click({$d=New-Object Windows.Forms.OpenFileDialog;$d.InitialDirectory=$macroDir;$d.Filter='Macro (*.json)|*.json';if($d.ShowDialog()-eq'OK'){try{$o=Get-Content -Raw -Encoding UTF8 $d.FileName|ConvertFrom-Json;if($o.schemaVersion-ne2){throw 'Старий формат макросу'};$name.Text=$o.name;$target.Text=$o.targetProcess;$grid.Rows.Clear();foreach($r in $o.steps){[void]$grid.Rows.Add($r.command,$r.argument,$r.description)};Set-Status 'ВІДКРИТО' $good}catch{Set-Status $_.Exception.Message $bad}}})

$script:steps=@();$script:index=0;$script:running=$false;$script:next=[DateTime]::MinValue;$script:held=New-Object Collections.Generic.HashSet[string]
...
$run.Add_Click({if($script:running){return};try{$script:steps=@(Compile);if(-not$script:steps.Count){throw 'Додайте хоча б одну дію'};$p=@(Get-Process -Name $target.Text.Trim() -ErrorAction SilentlyContinue|Where-Object MainWindowHandle -ne 0|Select-Object -First 1);if(-not$p){throw "Не знайдено вікно $($target.Text)"};if($focus.Checked){[MNative]::ShowWindowAsync($p[0].MainWindowHandle,9)|Out-Null;[MNative]::SetForegroundWindow($p[0].MainWindowHandle)|Out-Null};$script:index=0;$script:next=[DateTime]::Now.AddMilliseconds(500);$script:running=$true;Set-Status 'СТАРТ ЧЕРЕЗ 0.5 С' $gold2}catch{Set-Status $_.Exception.Message $bad}});$stop.Add_Click({Stop-Run});$form.Add_FormClosing({if($script:running){Stop-Run}})
