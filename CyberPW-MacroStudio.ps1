...
function Start-Run{if($script:running){return};try{$script:steps=@(Compile);if(-not$script:steps.Count){throw 'Додайте хоча б одну дію'};$p=@(Get-Process -Name $target.Text.Trim() -ErrorAction SilentlyContinue|Where-Object MainWindowHandle -ne 0|Select-Object -First 1);if(-not$p){throw "Не знайдено вікно $($target.Text)"};if($focus.Checked){[MNative]::ShowWindowAsync($p[0].MainWindowHandle,9)|Out-Null;[MNative]::SetForegroundWindow($p[0].MainWindowHandle)|Out-Null};$script:index=0;$script:next=[DateTime]::Now.AddMilliseconds(500);$script:running=$true;Set-Status "СТАРТ ЧЕРЕЗ 0.5 С · БІНД: $($script:startKey-replace'^D(?=\d)','')" $gold2}catch{Set-Status $_.Exception.Message $bad}}
...
$check.Add_Click({try{$s=Compile;Set-Status "СЦЕНАРІЙ КОРЕКТНИЙ`r`nКоманд: $($s.Count)" $good}catch{Set-Status $_.Exception.Message $bad}});$bind.Add_Click({$k=Pick-Key;if($k){if($k-eq'F12'){Set-Status 'F12 ЗАРЕЗЕРВОВАНО ДЛЯ СТОПУ' $bad;return};$script:startKey=$k;$script:startWasDown=$false;$bind.Text="БІНД · $($k-replace'^D(?=\d)','')";Set-Status "СТАРТ: $($bind.Text)" $good}});$new.Add_Click({if(-not$script:running){$name.Text='Новий макрос';$script:startKey='F10';$script:startWasDown=$false;$bind.Text='БІНД · F10';$grid.Rows.Clear();Set-Status 'НОВИЙ СЦЕНАРІЙ' $good}})
...
$open.Add_Click({$d=New-Object Windows.Forms.OpenFileDialog;$d.InitialDirectory=$macroDir;$d.Filter='Macro (*.json)|*.json';if($d.ShowDialog()-eq'OK'){try{$o=Get-Content -Raw -Encoding UTF8 $d.FileName|ConvertFrom-Json;if($o.schemaVersion-notin@(2,3)){throw 'Непідтримуваний формат макросу'};$name.Text=$o.name;$target.Text=$o.targetProcess;$loadedHotkey=if($o.PSObject.Properties['startHotkey']){[string]$o.startHotkey}else{'F10'};if(-not $loadedHotkey -or $loadedHotkey -eq 'F12' -or -not [Enum]::IsDefined([Windows.Forms.Keys],$loadedHotkey)){$loadedHotkey='F10'};$script:startKey=$loadedHotkey;$script:startWasDown=$false;$bind.Text="БІНД · $($loadedHotkey-replace'^D(?=\d)','')";$grid.Rows.Clear();foreach($r in $o.steps){[void]$grid.Rows.Add($r.command,$r.argument,$r.description)};Set-Status "ВІДКРИТО · $($bind.Text)" $good}catch{Set-Status $_.Exception.Message $bad}}})
