...
namespace CyberPW.Assistant2
{
    internal sealed class MacroRunner
    {
        ...
        public void Run(List<MacroInstruction> steps,string processName,bool focus)
        {
            ...
            {
                MacroInstruction step=steps[index];string[] parts=(step.Argument??"").Split(',');
                switch(step.Command)
                {
                    ...
                    case "CLICK": MacroNative.MouseButton(false,false);MacroNative.MouseButton(false,true);break;
        ...
        bool PixelMatches(string[] p){if(p.Length<3)throw new InvalidOperationException("Піксель: X,Y,#RRGGBB");Color actual=MacroNative.Pixel(ParseInt(p[0],-100000,100000),ParseInt(p[1],-100000,100000));Color expected=ColorTranslator.FromHtml(p[2].Trim());return actual.R==expected.R&&actual.G==expected.G&&actual.B==expected.B;}
